    ...
    if cls.resolve_macros:
        ...
        def tab(self, tabnum):
            ...
            if results is None:
                ...
                return results.replace(ranger.MACRO_DELIMITER, ranger.MACRO_DELIMITER_ESC)
            elif hasattr(results, '__iter__'):
                return (result.replace(ranger.MACRO_DELIMITER, ranger.MACRO_DELIMITER_ESC) for result in results)
            return None
        setattr(cls, 'tab', tab)
...
class CommandContainer(ranger.core.shared.FileManagerAware):
    def __init__(self):
        self.commands = {}
