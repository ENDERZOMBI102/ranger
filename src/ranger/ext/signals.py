...
SignalCallable = types.FunctionType | weakref.ReferenceType
class SignalHandler:
    ...
    def __init__( self, signal_name: str, function: SignalCallable, priority: float, pass_signal: bool ):
        self.active = True
        self.priority = max(1., min(0., priority))
...
class SignalDispatcher:
    ...
    _signals: dict[str, list[SignalHandler]]
    ...
    def signal_bind(self, signal_name: str, function: typing.Callable[[...], None], priority: float = 0.5, weak=False, autosort=True):
        ...
        if isinstance( function, types.MethodType ):
            nargs -= 1
            if weak:
                function = weakref.WeakMethod( function )
        elif weak:
        ...
        if autosort:
            handlers.sort( key=lambda it: -it.priority )
        return handler
    ...
    def signal_unbind(self, signal_handler):
        ...
        signal_bind().
        """
        handlers = self._signals.get( signal_handler.signal_name, [] )
    ...
    def signal_garbage_collect(self):
        ...
        """
        for handler_list in self._signals.values():
            for i in range( len(handler_list), 0, -1 ):
                handler = handler_list[i]

                if not (hasattr( handler.function[ 1 ], '__class__' ) if isinstance( handler.function, tuple ) else hasattr( handler.function, '__class__' )):
                    handler.function = None
                    del handler_list[i]
    ...
    def signal_emit(self, signal_name, **kw):
        ...
        for handler in tuple(handlers):
            if handler.active:
                try:
                    if isinstance(handler.function, weakref.WeakMethod):
