...
class Rifle:  # pylint: disable=too-many-instance-attributes
    delimiter1 = '='
    delimiter2 = ','
    ...
    def get_mimetype(self, fname):
        ...
        if not self._mimetype:
            try:
                with subprocess.Popen( ["file", "--mime-type", "-Lb", fname], stdout=PIPE, stderr=PIPE ) as process:
            ...
            if self._mimetype == 'application/octet-stream':
                try:
                    with subprocess.Popen( ["mimetype", "--output-format", "%m", fname], stdout=PIPE, stderr=PIPE ) as process:
                        mimetype, _ = process.communicate()
                    self._mimetype = mimetype.decode(ENCODING).strip()
                except OSError:
                    pass
        ...
        if command is None:  # pylint: disable=too-many-nested-blocks
            ...
            try:
                ...
                if 'f' in flags or 't' in flags:
                    ...
                    with subprocess.Popen( cmd, env=self.hook_environment(os.environ) ) as process:
...
def main():  # pylint: disable=too-many-locals
    ...
    if options.w is not None and not options.l:
        with subprocess.Popen([options.w] + list(positional)) as process:
            exit_code = process.wait()
    else:
        # Start up rifle
