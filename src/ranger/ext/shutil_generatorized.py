...
def copyfile(src, dst):
    ...
    with open(src, 'rb') as fsrc:
        with open(dst, 'wb') as fdst:
            try:
                yield from copyfileobj_range(fsrc, fdst)
            except OSError:
            ...
            yield from copyfileobj(fsrc, fdst)
...
def move(src, dst, overwrite=False, make_safe_path=get_safe_path):
    ...
    try:
        ...
        if os.path.isdir(src):
            ...
            yield from copytree(src, real_dst, symlinks=True, overwrite=overwrite, make_safe_path=make_safe_path)
            rmtree(src)
        else:
            yield from copy2(src, real_dst, symlinks=True, overwrite=overwrite, make_safe_path=make_safe_path)
            os.unlink(src)
