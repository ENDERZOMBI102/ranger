...
import ranger
import ranger.gui.color
import ranger.gui.context
import ranger.core.main
import ranger.ext.iter_tools
...
class ColorScheme:
    ...
    def get(self, *keys):
        ...
        """
        context = ranger.gui.context.Context(keys)
        color = self.use(context)
        if len(color) != 3 or not all(isinstance(value, int) for value in color):
            raise ValueError("Bad Value from colorscheme.  Need a tuple of (foreground_color, background_color, attribute).")
        return color
    ...
    def get_attr(self, *keys):
        ...
        """
        fg, bg, attr = self.get(*ranger.ext.iter_tools.flatten(keys))
        return attr | color_pair(ranger.gui.color.get_color(fg, bg))

    @abstractmethod
    def use(self, context) -> tuple[int, int, int]:
...
def _colorscheme_name_to_class(signal):  # pylint: disable=too-many-branches
    ...
    if scheme_supermodule is None:
        ...
        if usecustom:
            ranger.core.main.allow_access_to_confdir( ranger.args.confdir, True )
        scheme_module = getattr(
            __import__(scheme_supermodule, globals(), locals(), [scheme_name], 0), scheme_name)
        if usecustom:
            ranger.core.main.allow_access_to_confdir( ranger.args.confdir, False )
