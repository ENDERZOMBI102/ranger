...
import ranger.container.bookmarks
...
class FM(ranger.core.actions.Actions, ranger.ext.signals.SignalDispatcher):   # pylint: disable=too-many-instance-attributes
    input_blocked: bool = False
    input_blocked_until: float = 0
    mode: typing.Literal['normal', 'visual'] = 'normal'  # either 'normal' or 'visual'.
    search_method: typing.Literal['ctime'] = 'ctime'

    ui: ranger.gui.ui.UI | None
    start_paths: list[str]
    directories: dict
    bookmarks: ranger.container.bookmarks.Bookmarks
    current_tab: int
    tabs: dict[int, ranger.core.tab.Tab]
    tags: ranger.container.tags.Tags
    restorable_tabs: collections.deque
    previews: dict
    default_linemodes: collections.deque
    loader: ranger.core.loader.Loader
    copy_buffer: set
    do_cut: bool
    metadata: ranger.core.metadata.MetadataManager
    image_displayer: ranger.ext.img_display.ImageDisplayer | None
    run: ranger.core.runner.Runner | None
    rifle: ranger.ext.rifle.Rifle
    thistab: ranger.core.tab.Tab | None
    username: str
    hostname: str
    home_path: str
    mimetypes: mimetypes.MimeTypes
    thisfile: ranger.container.file.File
    thisdir: ranger.container.directory.Directory
    ...
    def __init__(self, ui=None, bookmarks=None, tags=None, paths=None):
        """Initialize FM."""
        ranger.core.actions.Actions.__init__(self)
        ranger.ext.signals.SignalDispatcher.__init__(self)
        ...
        self.tabs = {}
        self.tags = tags
        self.restorable_tabs = collections.deque( [], ranger.MAX_RESTORABLE_TABS )
        self.previews = {}
        self.default_linemodes = collections.deque()
        self.loader = Loader()
    ...
    def initialize(self):  # pylint: disable=too-many-statements
        ...
        set_image_displayer()
        self.settings.signal_bind('setopt.preview_images_method', set_image_displayer, priority=settings.SIGNAL_PRIORITY_AFTER_SYNC )

        self.settings.signal_bind( 'setopt.preview_images', lambda signal: signal.fm.previews.clear() )
        ...
        if self.bookmarks is None:
            ...
            self.bookmarks = ranger.container.bookmarks.Bookmarks( bookmarkfile=bookmarkfile, bookmarktype=Directory, autosave=self.settings.autosave_bookmarks )
            self.bookmarks.load()
        ...
        def sxiv_workaround_hook(command):
            ...
            if self.settings.open_all_images and not self.thisdir.marked_items and re.match(r'^(feh|n?sxiv|imv|pqiv) ', command):
                ...
                if images and self.thisfile.relative_path in images and "$@" in command:
                    ...
                    if command[0:5] == 'sxiv ' or command[0:6] == 'nsxiv ':
                        ...
                        new_command = command.replace("sxiv ", f"sxiv -n {number} ", 1 )

                    if command[0:4] == 'feh ':
                        new_command = command.replace( "feh ", f"feh --start-at {shell_quote(self.thisfile.relative_path)} ", 1 )

                    if command[0:4] == 'imv ':
                        number = images.index(self.thisfile.relative_path) + 1
                        new_command = command.replace("imv ", f"imv -n {number} ", 1 )

                    if command[0:5] == 'pqiv ':
                        number = images.index(self.thisfile.relative_path)
                        new_command = command.replace( "pqiv ", f"pqiv --action \"goto_file_byindex({number})\" ", 1 )

                    if new_command:
                        command = f"set -- {escaped_filenames}; {new_command}"
            return old_preprocessing_hook(command)

        self.rifle.hook_command_preprocessing = sxiv_workaround_hook

        def mylogfunc(text: str):
            self.notify(text, bad=True)
        self.run = ranger.core.runner.Runner( ui=self.ui, logfunc=mylogfunc, fm=self )
