...
@stack_filter("mime")
class MimeFilter(BaseFilter, ranger.core.shared.FileManagerAware):
    def __init__(self, pattern):
        self.regex = re.compile(pattern)
...
@stack_filter("hash")
class HashFilter(BaseFilter, ranger.core.shared.FileManagerAware):
    def __init__(self, filepath=None):
        ...
        self.filehash = list(ranger.ext.hash.hash_chunks(abspath(self.filepath)))

    def __call__(self, fobj):
        for (chunk1, chunk2) in zip_longest(self.filehash,
                                            ranger.ext.hash.hash_chunks(fobj.path),
                                            fillvalue=''):
            if chunk1 != chunk2:
                return False
        return True
...
def group_by_hash(fsobjects):
    ...
    for fobj in fsobjects:
        chunks = ranger.ext.hash.hash_chunks(fobj.path)
        chunk = next(chunks)
...
class DuplicateFilter(BaseFilter, ranger.core.shared.FileManagerAware):
    def __init__(self, _):
...
@stack_filter("unique")
class UniqueFilter(BaseFilter, ranger.core.shared.FileManagerAware):
    def __init__(self, _):
