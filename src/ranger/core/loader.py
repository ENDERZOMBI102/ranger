...
import ranger.ext.human_readable
import ranger.ext.safe_path
...
class CopyLoader(Loadable, ranger.core.shared.FileManagerAware):  # pylint: disable=too-many-instance-attributes
    progressbar_supported = True

    def __init__(self, copy_buffer, do_cut=False, overwrite=False, dest=None,
                 make_safe_path=ranger.ext.safe_path.get_safe_path):
    ...
    def generate(self):
        ...
        size_str = " (" + ranger.ext.human_readable.human_readable(self._calculate_size(1)) + ")"
        done = 0
        if self.do_cut:
...
class CommandLoader( Loadable, ranger.ext.signals.SignalDispatcher, ranger.core.shared.FileManagerAware ):
    ...
    def generate(self):
        ...
        popenargs['stdin'] = PIPE if self.input else open(os.devnull, 'r', encoding="utf-8")
    ...
    def work(self):
        ...
        while time() < end_time:
            try:
                ...
                self.fm.notify( f'Loader work process failed: {item.description} (Percent: {item.percent})', bad=True, exception=ex )
                self.old_item = None
                self._remove_current_process(item)
                break
        else:
