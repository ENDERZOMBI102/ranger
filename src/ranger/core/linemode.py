...
class LinemodeBase(ABC):
    ...
    Attributes:
        ...
        uses_metadata - True if metadata should be loaded for this linemode
...
class FileInfoLinemode(LinemodeBase):
    ...
    def infostring(self, fobj, metadata):
        if not fobj.is_directory:
            ...
            try:
                fileinfo = ranger.ext.spawn.check_output( [ "file", "-Lb", fobj.path ] ).strip()
...
class SizeMtimeLinemode(LinemodeBase):
    ...
    def infostring(self, fobj, metadata):
        ...
        if fobj.is_directory and not fobj.cumulative_size_calculated:
            ...
            sizestring = ranger.ext.human_readable.human_readable(fobj.size)
        return f"{sizestring} {datetime.fromtimestamp(fobj.stat.st_mtime).strftime( "%Y-%m-%d %H:%M" )}"
...
class HumanReadableMtimeLinemode(LinemodeBase):
    ...
    def infostring(self, fobj, metadata):
        if fobj.stat is None:
            return '?'
        return ranger.ext.human_readable.human_readable_time(fobj.stat.st_mtime)
...
class SizeHumanReadableMtimeLinemode(LinemodeBase):
    ...
    def infostring(self, fobj, metadata):
        ...
        if fobj.is_directory and not fobj.cumulative_size_calculated:
            ...
            sizestring = ranger.ext.human_readable.human_readable(fobj.size)
        return "%s %11s" % (sizestring, ranger.ext.human_readable.human_readable_time(fobj.stat.st_mtime))
