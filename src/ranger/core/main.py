...
VERSION_MSG: list[str] = [
    f'ranger version: {ranger.VERSION}',
    f'Python version: {' '.join(line.strip() for line in sys.version.splitlines())}',
    f'Locale: {'.'.join(str(s) for s in locale.getlocale())}',
]


def main() -> int:  # pylint: disable=too-many-locals,too-many-return-statements,disable=too-many-branches,too-many-statements
    """initialize objects and run the filemanager"""
    import ranger.container.settings
    import ranger.core.shared
    import ranger.core.fm
    import ranger.ext.logutils
    import ranger.ext.openstruct

    ranger.args = args = parse_arguments()
    ranger.arg = ranger.ext.openstruct.OpenStruct( args.__dict__ )  # COMPAT
    ranger.ext.logutils.setup_logging(debug=args.debug, logfile=args.logfile)
    ...
    if args.list_tagged_files:
        ...
        fm = ranger.core.fm.FM()
        try:
            with open( fm.datapath('tagged'), 'r', encoding="utf-8", errors='replace' ) as fobj:
    ...
    ranger.core.shared.SettingsAware.settings_set(ranger.container.settings.Settings())
    ...
    if paths_inaccessible:
        print( 'Inaccessible paths: ', ', '.join(paths_inaccessible), file=sys.stderr)
        return 1
    ...
    try:  # pylint: disable=too-many-nested-blocks
        # Initialize objects
        fm = ranger.core.fm.FM(paths=paths)
        ...
        exit_msg += '\n'.join(VERSION_MSG) + '\n'
        try:
            exit_msg += f"Current file: {repr(fm.thisfile.path)}\n"
        except Exception:  # pylint: disable=broad-except
            pass
        exit_msg += \
f'''
...
def parse_arguments():
    ...
    def path_init(option):
        ...
        try:
            ...
            sys.stderr.write( f'--{option} is not accessible: {argval}\n{ex}\n' )
            sys.exit(1)
        if os.path.exists(path) and not os.access(path, os.W_OK):
            sys.stderr.write( f'--{option} is not writable: {path}\n' )
            sys.exit(1)
        return path
    ...
    if args.clean:
        ...
        def cleanup_cachedir():  # pylint: disable=unused-variable
            try:
                ...
                sys.stderr.write( f"Error during the temporary cache directory cleanup:\n{ex}\n" )
    else:
...
def load_settings( fm, clean ):  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    ...
    if not clean:
        ...
        try:
            ...
            if not os.path.exists(fm.confpath('plugins', '__init__.py')):
                ...
                with open( fm.confpath('plugins', '__init__.py'), 'w', encoding="utf-8" ):
        ...
        if os.environ.get( 'RANGER_LOAD_DEFAULT_RC', 'TRUE' ).upper() != 'FALSE' or not (custom_conf_is_readable or system_conf_is_readable):
